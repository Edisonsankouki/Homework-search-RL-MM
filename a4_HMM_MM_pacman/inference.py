...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        p_noisy_true = busters.getObservationProbability(noisyDistance,true_distance)
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        all_position = self.allPositions
        result = DiscreteDistribution()
        for oldPos in all_position:
            newPosDict = self.getPositionDistribution(gameState,oldPos)
            for newPos,prob in newPosDict.items():
                result[newPos] += self.beliefs[oldPos] * prob # new = old*prob
        self.beliefs = result
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        actions = self.legalPositions
        unit_particles = self.numParticles/len(actions)
        for action in actions:
            for i in range(int(unit_particles)):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for i in range(self.numParticles):
            self.particles.append(result.sample())
...
class JointParticleFilter(ParticleFilter):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        new_result  = DiscreteDistribution()
        for particle in self.particles:
            prob = 1.0
            for i in range(self.numGhosts):
                ...
                if ghostDist is not None:
                    prob *= self.getObservationProb(ghostDist, gameState.getPacmanPosition(), particle[i], jailPos)
                else:
                    particle = list(particle)
                    particle[i] = jailPos
                    particle = tuple(particle)
                    prob = 0
            new_result[particle] += prob
        new_result.normalize()

        if new_result.total() == 0:
            self.initializeUniformly(gameState)
            new_result = self.getBeliefDistribution()

        self.particles = []
        for i in range(self.numParticles):
            self.particles.append(new_result.sample())
